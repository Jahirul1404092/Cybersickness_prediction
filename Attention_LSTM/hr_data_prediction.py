# -*- coding: utf-8 -*-
"""
Created on Wed Oct 11 15:28:42 2023
...
import Attention_layer as Attention_layer
model = Sequential()
model.add(LSTM(64, activation='relu', input_shape=(X_train_hr.shape[1], X_train_hr.shape[2]), return_sequences=True, dropout=0.3))
model.add(Attention_layer.attention(return_sequences=True))
model.add(LSTM(32, activation='relu', return_sequences=True, dropout=0.2))
model.add(Attention_layer.attention(return_sequences=True))
model.add(LSTM(32, activation='relu', return_sequences=True, dropout=0.3))
model.add(Attention_layer.attention(return_sequences=True))
model.add(LSTM(16, activation='relu', return_sequences=True, dropout=0.2))
model.add(Attention_layer.attention(return_sequences=True))
model.add(LSTM(10, activation='relu', return_sequences=False, dropout=0.1))
#model.add(Dropout(0.2))
model.add(Dense(10))
model.add(Dense(Y_train_hr.shape[1]))
...
y_pred_hr = model.predict(X_test_hr)
print(y_pred_hr.shape, Y_test_hr.shape)
import math
from sklearn.metrics import mean_squared_error, mean_absolute_error, r2_score , confusion_matrix
from scipy.stats import pearsonr
results = {}
results['dataset_name'] = "hr"
results['mean_squared_error'] = round(mean_squared_error(Y_test_hr[:,-1], y_pred_hr[:,-1]),3)
results['root_mean_squared_error'] = round(math.sqrt(mean_squared_error(Y_test_hr[:,-1], y_pred_hr[:,-1])),3)
results['mean_absolute_error'] = round(mean_absolute_error(Y_test_hr[:,-1], y_pred_hr[:,-1]),3)
results['r2_score'] = round(r2_score(Y_test_hr[:,-1], y_pred_hr[:,-1]),3)
results['smape'] = round(smape(Y_test_hr[:,-1], y_pred_hr[:,-1]),3)
pcc, _ = pearsonr(Y_test_hr[:,-1], y_pred_hr[:,-1])
...
try:
    ...
    pass
result_df.to_csv(os.path.join(save_dir, f'hr_data_prediction_metrics.csv'), index = False, header=True)
