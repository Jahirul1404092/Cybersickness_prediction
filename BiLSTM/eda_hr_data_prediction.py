...
from tensorflow.keras.layers import LSTM, concatenate, Bidirectional
...
save_dir=cwd+"\\eda_hr_pred_folder"
...
model1.add(Bidirectional(forward_layer, backward_layer=backward_layer, input_shape=(X_train_eda.shape[1], X_train_eda.shape[2])))
model1.add(LSTM(128, activation='relu', return_sequences=True, dropout=0.3))
...
#model1.add(Dense(10))
#model1.add(Dense(Y_train_eda.shape[1]))
model1.summary()

model2 = Sequential()
forward_layer = LSTM(128, return_sequences=True)
backward_layer = LSTM(128, activation='relu', return_sequences=True, go_backwards=True)
model2.add(Bidirectional(forward_layer, backward_layer=backward_layer, input_shape=(X_train_hr.shape[1], X_train_hr.shape[2])))
model2.add(LSTM(128, activation='relu', return_sequences=True, dropout=0.3))
...
cp_callback = ModelCheckpoint(filepath=checkpoint_path,
                                        ...
                                        verbose=1)
start = time.time()
history = combined_model.fit([X_train_eda, X_train_hr], y=Y_train_hr, epochs=100, batch_size=64, validation_data=([X_val_eda,X_val_hr], Y_val_hr), verbose=1, callbacks=[cp_callback])
...
y_pred_eda_hr = combined_model.predict([X_test_eda, X_test_hr])
print(y_pred_eda_hr.shape, Y_test_eda.shape)
import math
from sklearn.metrics import mean_squared_error, mean_absolute_error, r2_score , confusion_matrix
from scipy.stats import pearsonr
results = {}
results['dataset_name'] = "eda_hr"
results['mean_squared_error'] = round(mean_squared_error(Y_test_eda[:,-1], y_pred_eda_hr[:,-1]),3)
results['root_mean_squared_error'] = round(math.sqrt(mean_squared_error(Y_test_eda[:,-1], y_pred_eda_hr[:,-1])),3)
results['mean_absolute_error'] = round(mean_absolute_error(Y_test_eda[:,-1], y_pred_eda_hr[:,-1]),3)
results['r2_score'] = round(r2_score(Y_test_eda[:,-1], y_pred_eda_hr[:,-1]),3)
results['smape'] = round(smape(Y_test_eda[:,-1], y_pred_eda_hr[:,-1]),3)
pcc, _ = pearsonr(Y_test_eda[:,-1], y_pred_eda_hr[:,-1])
...
try:
    ...
    pass
result_df.to_csv(os.path.join(save_dir, f'eda_hr_data_prediction_metrics.csv'), index = False, header=True)